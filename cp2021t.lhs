...
g_eval_exp a = either (const a) num_ops where
	...
	ops = either bin un
	bin(Sum,(a,b)) = uncurry (+) (a,b)
	bin(Product,(a,b)) = uncurry (*) (a,b)
	un(Negate,a) = negate a
	un(E,a) = expd a	
---
clean = outExpAr . c . outExpAr 
  c = either (const X) num_ops where 
      num_ops = either N ops
	    ops = either bin un
	    bin(Product,(N 0,b)) = N 0
	    bin(Product,(a, N 0)) = N 0
	    bin(op,(a,b)) = Bin op a b
	    un(E,N 0) = N 1
	    un(op,a) = Un op a
---
...
&catdef (n+1) = \frac{(2*n+2)!}{((n+2)!((n+1)!)}\\
&catdef (n+1) = \frac{(2*n+2)(2*n+1)(2*n)!}{((n+2)(n+1)!((n+1)(n!))}\\
&catdef (n+1) = \frac{(2*n+2)(2*n+1)}{(n+2)(n+1)!} * \frac{(2*n)!}{(n+1)!n!}\\
&catdef (n+1) = \frac{2*(n+1)(2*n+1)}{(n+2)(n+1)} * \frac{(2*n)!}{(n+1)!n!}\\
&catdef (n+1) = \frac{2*(2*n+1)}{n+2} * catdef n
\end{eqnarray*}
...
cataAvg g = g . (id -|- (id >< (cataAvg g ))) . outL 
g = split h k 
h = either (id) ( (uncurry (/)) . split ((uncurry (+)) . (id >< (uncurry (*)))) (succ . p2 . (id >< p2)) )
k = either (const 1) (succ . p2 . (id >< p2))
outL [a] = i1(a)
outL (a:x) = i2(a,x)
\end{code}
...
avgLTree = p1.cataLTree gene where
   gene = split h k
   h = either (id) ( (uncurry (/)) . split ( (uncurry (+)) . ( ( (uncurry (*)) >< (uncurry (*)) ) ) ) ( (uncurry (+)) . (p2 >< p2) ) )
   k = either (const 1) (uncurry (+) . (p2 >< p2))
\end{code}

Resolução: 

Diagrama de length para listas não vazias:
\begin{eqnarray*}
\xymatrix@@C=2cm{
    |A*|
           \ar[d]_-{|cata (g)|}
...
Partindo agora para o calculo do gene(k) de length:
\begin{eqnarray*}
\start
          |lcbr(
    length [a] = 1
...
%
\just\equiv{ Definição de singl, definição de cons e definição de succ}
%
          |lcbr(
    length . singl a = 1
  )(
    length . cons (a,x) = succ . (length x)
  )|
%
\just\equiv{Def-const (74), Def-proj (79)}
%
	|lcbr(
    length . singl a = const 1 a
  )(
    length . cons (a,x) = succ . length . p2 (a,x)
  )|
%
...
%
    ...
    length . cons = succ . length . p2
  )|
%
\just\equiv{Eq-+ (27)}
%
  |either (length . singl) (length . cons) = either (const 1) (succ . length . p2)|
%
\just\equiv{Fusão-+ (20)}
%
  |length . either singl cons = either (const 1) (succ . length . p2)|
%
\just\equiv{Natural-id (1), Natural-p2 (13)}
%
  |length . either singl cons = either (const 1 . id) (succ . p2 . (id >< length) )|
%
\just\equiv{ Cancelamento-|><| (7), Natural-id (1)}
%
  |length . either singl cons = either (const 1 . id) (succ . p2 . (id . id >< p2 . split avg length) )|
%
\just\equiv{ Functor-|><| (14) }
%
  |length . either singl cons = either (const 1 . id) (succ . p2 . (id >< p2 . id >< split avg length) )|
%
...
%
  |length . either singl cons = either (const 1) (succ . p2 . (id >< p2)) . (id + id >< split avg length)|
%
\just\equiv{ Definição de in, Definição de Functor de listas }
%
  |length . in = either (const 1) (succ . p2 . (id >< p2)) . F (split avg length)|
\qed
\end{eqnarray*}

Logo, k = |either (const 1) (succ . p2 . (id >< p2))|;


Diagrama de avg para listas não vazias:
\begin{eqnarray*}
\xymatrix@@C=2cm{
    |A*|
           \ar[d]_-{|cata (g)|}
...
\begin{spec}
  avg [a] = a
  avg (a:x) = frac( a + (length x)*(avg x) )( (length x) + 1 )
\end{spec}
...
Partindo agora para o calculo do gene(h) de avg:
...
\start
    ...
    avg (a:x) = frac( a + (avg x) * (length x) )( (length x) + 1)
  )| 
%
\just\equiv{ Definição de singl, definição de cons e definição de succ}
%
    ...
    avg . cons (a,x) = frac( a + (avg x) * (length x) )( succ . (length x))
  )|
%
\just\equiv{ Def-id (73), Uncurry (84)}
%
          |lcbr(
    avg . singl a = id a
  )(
    avg . cons (a,x) = frac( uncurry (+) ( a, uncurry (*) ((avg x),(length x)) ) )( succ . (length x)))
  )|
%
\just\equiv{Def-split (76), Def-id (73)}
%
          |lcbr(
    avg . singl a = id a
  )(
    avg . cons (a,x) = frac( uncurry (+) (id a, (uncurry (*)) . split avg length x) )( succ . (length x) )
  )|
%
\just\equiv{Def-|><| (77), Def-proj (79) }
%
          |lcbr(
    avg . singl a = id a
  )(
    avg . cons (a,x) = frac ( uncurry (+) . (id >< ((uncurry (*)) . split avg length)) (a,x) )( succ . length . p2 (a,x)  )
  )|
%
...
%
    ...
    avg . cons (a,x) = uncurry (/) ( uncurry (+) . (id >< ((uncurry (*)) . split avg length)) (a,x) , succ . length . p2 (a,x)  )
  )|
%
\just\equiv{ Def-split (76), Igualdade extensional (71)}
%
          |lcbr(
    avg . singl = id
  )(
    avg . cons = uncurry (/) . split ( uncurry (+) . (id >< ((uncurry (*)) . split avg length)) ) (succ . length . p2 )
  )|
%
\just\equiv{Eq-+ (27)}
%
  |either (avg . singl) (avg.cons) = either (id) ( uncurry (/) . split ( uncurry (+) . (id >< ((uncurry (*)) . split avg length)) ) (succ . length . p2 ) )|
%
\just\equiv{fusão-+ (20)}
%
  |avg . either singl cons = either (id) ( uncurry (/) . split ( uncurry (+) . (id >< ((uncurry (*)) . split avg length)) ) (succ . length . p2 ) )|
%
\just\equiv{ Natural-id (1); Natural p2 (13)}
%
  |avg . either singl cons = either (id) ( uncurry (/) . split ( uncurry (+) . ((id . id) >< ((uncurry (*)) . split avg length)) ) (succ . p2 . (id >< length) ) )|
%
\just\equiv{ Functor-|><| (14)}
%
  |avg . either singl cons = either (id) ( uncurry (/) . split ( uncurry (+) . ( (id >< (uncurry (*))) . (id >< split avg length) ) ) (succ . p2 . (id >< length) )  )|
%
\just\equiv{Natural-id (1), Cancelamento-|><| (7)}
%
  |avg . either singl cons = either (id) ( uncurry (/) . split ( uncurry (+) . ( (id >< (uncurry (*))) . (id >< split avg length) ) ) (succ . p2 . (id . id >< p2 . split avg length) ) )|
%
\just\equiv{Functor-|><| (14)}
%
  |avg . either singl cons = either (id) ( uncurry (/) . split ( uncurry (+) . ( (id >< (uncurry (*))) . (id >< split avg length) ) ) 
                                                             (succ . p2 . ((id >< p2) . (id >< split avg length)) ) )|
%
\just\equiv{Assoc-comp (2)}
%
  |avg . either singl cons = either (id) ( uncurry (/) . split ( uncurry (+) . ( (id >< (uncurry (*))) . (id >< split avg length) ) )
...
\just\equiv{Fusão-|><| (9)}
%
  |avg . either singl cons = either (id) ( uncurry (/) . split ( uncurry (+) . (id >< (uncurry (*))) ) (succ . p2 . (id >< p2) ) . (id >< split avg length))|
%
\just\equiv{ Natural-id (1), Absorção-+ (22)}
%
  |avg . either singl cons = either (id) ( uncurry (/) . split ( uncurry (+) . (id >< (uncurry (*))) ) (succ . p2 . (id >< p2) ) ) . (id + id >< split avg length)|
%
\just\equiv{ Definição de in, definição de Functor de listas }
%
  |avg . in = either (id) ( uncurry (/) . split ( uncurry (+) . (id >< (uncurry (*))) ) (succ . p2 . (id >< p2) ) ) . F (split avg length)|

\qed
\end{eqnarray*}

Logo, h = |either (id) ( uncurry (/) . split ( uncurry (+) . (id >< (uncurry (*))) ) (succ . p2 . (id >< p2) ) )|;
...
%
  |split (avg) (length) = cata (split (either (id) ( uncurry (/) . split ( uncurry (+) . (id >< (uncurry (*))) ) (succ . p2 . (id >< p2) ) )) (either (const 1) (succ . p2 . (id >< p2))))|
%  
\just\equiv{Lei da troca (28)}
%
  |split (avg) (length) = cata (either (split (id) (const 1)) (split (uncurry (/) . split ( uncurry (+) . (id >< (uncurry (*))) ) (succ . p2 . (id >< p2) )) (succ . p2 . (id >< p2))))|
 
\qed
\end{eqnarray*}


Generalizando o raciocínio anterior para o cálculo da média de todos os elementos de uma LTree (recorrendo a uma unica travessia da árvore)
e tendo por base o seguinte diagrama:
...
Partindo agora para o calculo do gene(k) de length para LTree:
...
\start
    ...
    length Fork (t1,t2) = (length t1) + (length t2)
  )|
%
\just\equiv{ Def-comp (72), Def-const (74)}
%
          |lcbr(
    length . Leaf a = const 1 a
  )(
    length . Fork (t1,t2) = (length t1) + (length t2)
  )|
%
...
%
    ...
    length . Fork (t1,t2) = uncurry (+) (length t1, length t2)
  )|
%
\just\equiv{ Def-|><| (77) }
%
    ...
    length . Fork (t1,t2) = uncurry (+) . (length >< length) (t1,t2)
  )|
%
\just\equiv{ Igualdade extensional (71)}
%
    ...
    length . Fork = uncurry (+) . (length >< length) 
  )|
%
\just\equiv{Eq-+ (27), Fusão-+ (20)}
%
  |length . either Leaf Fork = either (const 1) (uncurry (+) . (length >< length))|
%
\just\equiv{ Cancelamento-|><| (7)}
%
  |length . either Leaf Fork = either (const 1) (uncurry (+) . (p2 . split avg length) >< (p2 . split avg length))|
%
\just\equiv{ Natural-id (1); Functor-|><| (14) }
%
  |length . either Leaf Fork = either (const 1 . id) (uncurry (+) . (p2 >< p2) . (split avg length >< split avg length))|
%
\just\equiv{ Absorção-+ (22) }
%
  |length . either Leaf Fork = either (const 1) (uncurry (+) . (p2 >< p2)) . (id + (split avg length >< split avg length))|
%
\just\equiv{ Definição de in, Definição de Functor de LTree }
%
  |length . in = either (const 1) (uncurry (+) . (p2 >< p2)) . F(split avg length)|

\qed
\end{eqnarray*}

Logo, k = |either (const 1) (uncurry (+) . (p2 >< p2))|;


Partindo agora para o calculo do gene(k) de avg para Ltree:
...
\start
    ...
    avg Fork (t1,t2) = frac( (avg t1) * (length t1)  +  (avg t2) * (length t2) )( (length t1) + (length t2) )
  )|
%
\just\equiv{ Def-id (73), Def-comp (72), Uncurry (84) }
%
          |lcbr(
    avg . Leaf a = id a
  )(
    avg . Fork (t1,t2) = 
    frac( uncurry (+) (uncurry (*) ((avg t1),(length t1)),uncurry (*) ((avg t2),(length t2))))( uncurry (+) ((length t1), (length t2)) )
  )|
%
\just\equiv{ Def-split (76), Def-|><| (77)}
%
          |lcbr(
    avg . Leaf a = id a
  )(
    avg . Fork (t1,t2) = 
    frac( (uncurry (+)) ((uncurry (*)) . (split avg length t1),(uncurry (*)) . (split avg length t2 )))( (uncurry (+)) . (length >< length) (t1,t2)  )
  )|
%
\just\equiv{ Def-|><| (77)}
%
    ...
    avg . Fork (t1,t2) = 
    frac( (uncurry (+)) . ( ((uncurry (*)) . split avg length) ><  ((uncurry (*)) . split avg length) ) (t1,t2) )( (uncurry (+)) . (length >< length) (t1,t2) )
  )|
%
\just\equiv{ Uncurry (84)}
%
    ...
    avg . Fork (t1,t2) = (uncurry (/)) . ( (uncurry (+)) . ( ((uncurry (*)) . split avg length) ><  ((uncurry (*)) . split avg length) ) (t1,t2) )( (uncurry (+)) . (length >< length) (t1,t2) )
  )|
%
...
%
    ...
    avg . Fork (t1,t2) = (uncurry (/)) . split ( (uncurry (+)) . ( ((uncurry (*)) . split avg length) ><  ((uncurry (*)) . split avg length) ) )( (uncurry (+)) . (length >< length) ) (t1,t2)
  )|
%
\just\equiv{Igualdade extensional (71)}
%
    ...
    avg . Fork = (uncurry (/)) . split ( (uncurry (+)) . ( ((uncurry (*)) . split avg length) ><  ((uncurry (*)) . split avg length) ) )( (uncurry (+)) . (length >< length) )
  )|
%
\just\equiv{Eq-+ (27)}
%
  |either (avg . Leaf) (avg . Fork) = either (id) ( (uncurry (/)) . split ( (uncurry (+)) . ( ((uncurry (*)) . split avg length) ><  ((uncurry (*)) . split avg length) ) )( (uncurry (+)) . (length >< length) ) )|
%
\just\equiv{ Fusão-+ (20)}
%
  |avg . either Leaf Fork = either (id) ( (uncurry (/)) . split ( (uncurry (+)) . ( ((uncurry (*)) . split avg length) ><  ((uncurry (*)) . split avg length) ) )( (uncurry (+)) . (length >< length) ) )|
%
\just\equiv{Functor-|><| (14)}
%
  |avg . either Leaf Fork = either (id) ( (uncurry (/)) . split ( (uncurry (+)) . ( ( (uncurry (*)) >< (uncurry (*)) ) . (split avg length >< split avg length)) ) ( (uncurry (+)) . (length >< length) ) )|
%
\just\equiv{Cancelamento-|><| (7)}
%
  |avg . either Leaf Fork = either (id) ( (uncurry (/)) . split ( (uncurry (+)) . ( ( (uncurry (*)) >< (uncurry (*)) ) . (split avg length >< split avg length)) ) ( (uncurry (+)) . (p2 . split avg length >< p2 . split avg length) ) )|
%
\just\equiv{ Functor-|><| (14)}
%
  |avg . either Leaf Fork = either (id) ( (uncurry (/)) . split ( (uncurry (+)) . ( ( (uncurry (*)) >< (uncurry (*)) ) . (split avg length >< split avg length)) ) ( (uncurry (+)) . ((p2 >< p2) . (split avg length >< split avg length)) ) )|
%
\just\equiv{ Fusão-|><| (9), Natural-id (1)}
%
  |avg . either Leaf Fork = either (id . id) ( (uncurry (/)) . split ( (uncurry (+)) . ( ( (uncurry (*)) >< (uncurry (*)) ) ) ) ( (uncurry (+)) . (p2 >< p2) ) . (split avg length >< split avg length) )|
%
\just\equiv{ Absorção-+ (22)}
%
  |avg . either Leaf Fork = either (id) ( (uncurry (/)) . split ( (uncurry (+)) . ( ( (uncurry (*)) >< (uncurry (*)) ) ) ) ( (uncurry (+)) . (p2 >< p2) ) ) . (id + (split avg length >< split avg length))|
%
\just\equiv{ Definição de in, Definição de Functor de LTree}
%
  |avg . in = either (id) ( (uncurry (/)) . split ( (uncurry (+)) . ( ( (uncurry (*)) >< (uncurry (*)) ) ) ) ( (uncurry (+)) . (p2 >< p2) ) ) . F (split avg length)|
\qed
\end{eqnarray*}

Logo, h = |either (id) ( (uncurry (/)) . split ( (uncurry (+)) . ( ( (uncurry (*)) >< (uncurry (*)) ) ) ) ( (uncurry (+)) . (p2 >< p2) ) )|; 
...
\just\equiv{Fokkinga (52)}
%
  |split (avg) (length) = cata ( split (either (id) ( (uncurry (/)) . split ( (uncurry (+)) . ( ( (uncurry (*)) >< (uncurry (*)) ) ) ) ( (uncurry (+)) . (p2 >< p2) ) ))  (either (const 1) (uncurry (+) . (p2 >< p2))) )|
%  
\just\equiv{Lei da troca (28)}
%
  |split (avg) (length) = cata ( either ( split (id) (const 1) ) ( split ((uncurry (/)) . split ( (uncurry (+)) . ( ( (uncurry (*)) >< (uncurry (*)) ) ) ) ( (uncurry (+)) . (p2 >< p2) )) (uncurry (+) . (p2 >< p2)) ))|
